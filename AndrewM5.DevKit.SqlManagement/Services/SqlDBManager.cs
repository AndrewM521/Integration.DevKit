...
public class SqlDBManager : ISqlDBManager
{
    ...
    public SqlDBManager(IOptions<SqlDBManagerSettings> settings, ICustomLoggerManager loggerManager)
    {
        if (settings == null)
        {
        ...
        if (RuntimeSettings.MaxSqlDBClients < 0)
        {
            RuntimeSettings.MaxSqlDBClients = int.MaxValue;
        }
