# SQL Management Module: Quick Start
The SQL Management module provides a factory-based approach to handling SQL database interactions. It consists of the SqlDBManager, 
...
## 2. Setup
This module follows the same host-based initialization pattern as the rest of the DevKit. It involves registering the service and then initializing the static host provider.

### Registration
Add the SQL Management services to your IServiceCollection:
```
...
### Initalization
After building the host, Initialize the SqlManagementHost to enable access throughout your application:
```
...
The RunNonQueryCommandAsync has 2 overloads. 
1. Gives access to the command object using a custom callback
2. No access to the command object



method uses a callback. Your logic stays inside the block, and the module ensures the command is closed immediately afterward.
