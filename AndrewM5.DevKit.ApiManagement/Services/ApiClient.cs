...
public class ApiClient : IApiClient
{
    ...
    public ApiClient(IApiManager apiManager, string clientName, ApiClientSettings clientSettings, HttpClient httpClient, ICustomLogger? logger = null)
    {
        ...
        if (RuntimeSettings.TimeoutSeconds != null)
        {
            if (RuntimeSettings.TimeoutSeconds < 0)
            {
                RuntimeSettings.TimeoutSeconds = 0;
            }
        }
        else
        {
        ...
        if (RuntimeSettings.TimeoutSeconds != 0)
        {
            _httpClient.Timeout = TimeSpan.FromSeconds((double)RuntimeSettings.TimeoutSeconds);
        }
        else
        {
